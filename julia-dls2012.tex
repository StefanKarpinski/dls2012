...
convenience and productivity. Despite these advantages, C and Fortran
remain the gold standard languages for the most essential parts of
technical computing because high-level dynamic languages still lack
sufficient performance. As a result, the most challenging areas of
...
aforementioned dynamic technical computing environments are all instances
of this design. While this approach is pragmatic for many applications,
there are significant drawbacks. It would, of course, be preferable to
write compute-intensive code in a more productive language. This is
especially true when developing parallel algorithms, where code complexity
can increase dramatically. Programming in two languages can be more
complex than using either language by itself due to the need for mediation
between different type domains and incompatible memory management schemes.
These interfacing issues may also add overhead when calling between
layers. When such a system is used for mathematical programming there is
pressure to write ``vectorized'' code, which is not natural for every
problem. From a compiler's point of view, these designs make it difficult
to perform whole-program optimization. Two-tiered systems also present a
massive social barrier preventing the vast majority of users from
understanding or contributing to their internals, since they are written
in a different language hidden behind a complex API.
% In the case of proprietary systems, the barrier to contribution is even
% more extreme.
...
double precision}). It is cumbersome to write code that uses floating
point numbers with different precisions, or other types. Programs that use
user-defined types might run slowly, so that users tend to avoid defining
their own types altogether.
