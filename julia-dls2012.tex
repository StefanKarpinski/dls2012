...
The resulting architecture is two-tiered: the programmer expresses
high-level logic in a dynamic language while the heavy lifting is done in
C and Fortran. The aforementioned dynamic technical computing environments
are all instances of this design. While this approach is pragmatic for
many applications, there are significant drawbacks. It would be preferable
to also write compute-intensive code in a more productive language. This
is especially true when developing parallel algorithms, where code
complexity can increase dramatically. Programming in two languages can be
more complex than using either language by itself due to the need for
mediation between different type domains and incompatible memory
management schemes. These interfacing issues may also add overhead when
calling between layers. When such a system is used for mathematical
programming there is pressure to write ``vectorized'' code, which is not
natural for every problem. Lastly, from a compiler's point of view, these
designs make it difficult to perform whole-program optimization.
