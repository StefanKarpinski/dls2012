...
% are used for interaction and scripting overall workflow.  A new
% approach to dynamic language design can change this situation for
% technical computing, providing productivity and performance in one
...
user, the library developer, and the language designer. Users of such
languages overwhelmingly prefer features such as interactivity (REPL,
graphics, etc.), scripting, type-free programming, and high
performance to the extent possible. Many users go to extreme lengths
to avoid using the {\it two-tier trap}, where performance critical
sections of the code are rewritten in C or Fortran. Aggressive
vectorization is a commonly used technique that is used to avoid the
two-tier trap, even at the cost of making the code difficult to read,
and increasing memory utilization. More compute resources are also
often thrown at the problem to avoid rewriting the application.
Although seasoned programmers consider it a badge of honour, most
users simply do not know C or Fortran, are unwilling to learn, or
simply unwilling to go through the hassle. Programming in two
...
performance --- type annotations, parallelization, embracing the {\it
  two-tier trap}, etc. Rewriting computational kernels in a different
language often puts them at a disadvantage though, since the resulting
code is not generic, or uses a combination of perl or shell scripts to
generate code that can work with multiple input types. Distribution of
such libraries is also difficult, since they have to be compiled for
various combinations of operating systems and compilers.

The language designer prefers to design a language that exposes the
amount of low-level information to the compiler for high performance,
without curbing the productivity of the end users and library
developers. From a compiler's point of view, programming in multiple
languages makes it difficult to perform whole-program optimization. It
is difficult to perform domain-specific optimizations of C code, and
expressing algorithms at a higher level makes certain optimizations
easier.
...
language. This is obviously useful where a large body of existing code
can benefit from language-level performance enhancements, but while
promising, these efforts have yet to deliver. Julia is designed for
